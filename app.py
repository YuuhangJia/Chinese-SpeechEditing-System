...
def process():
    ...
    # 进行语音编辑：
    region_info = find_difference(original_text, target_text)
    ...
    commands = [f"export PYTHONPATH=.",
                f"python inference/tts/spec_denoiser_api.py --text \"{original_text}\" --edited_text \"{target_text}\" --region \"{region_info['region']}\" --edited_region \"{region_info['edited_region']}\" --exp_name spec_denoiser_ai3 --config egs/spec_denoiser_aishell3.yaml"
    ]
    ...
    if os.path.exists("inference/out/original_audio.wav"):
        ...
        return "输入的原文本与音频文件不一致！请检查是否存在英文、阿拉伯数字等暂不支持的符号。"
