...
class BaseBinarizer:
    ...
    def train_item_names(self):
        range_ = self._convert_range([0, -1])
        return self.item_names[range_[0]:range_[1]]

    @property
    def valid_item_names(self):
        range_ = self._convert_range([0, 0])
        return self.item_names[range_[0]:range_[1]]
    ...
    def process_item(cls, item, text2mel_params):
        ...
        wav_fn = item['wav_fn']
        wav, mel = cls.process_audio(f"data/raw/aishell3/{wav_fn}", item, text2mel_params) # jyh
        ...
        try:
            ...
            if text2mel_params['with_align']:
                tg_fn = f"data/processed/{text2mel_params['dataset_name']}/mfa_outputs/{item_name[:-4]}.TextGrid" # jyh: "SSB00050001.wav"
                item['tg_fn'] = tg_fn
